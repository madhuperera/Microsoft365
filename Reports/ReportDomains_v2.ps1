...
.NOTES
    ...
    DNS lookups use DNS-over-HTTPS (DoH) via Google and Cloudflare, so this
    script is cross-platform (macOS, Linux, Windows) and does not depend on the
...
# Runs on macOS, Linux and Windows without the Windows-only DnsClient module.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helper: resolve DNS safely. Thin wrapper over Resolve-DnsRecordDoH that keeps
# the original return contract (Records / AllRecords / Server / Error). The
# -PreferredServers parameter is retained for call-site compatibility but is
# not needed with DoH (the public resolvers are already authoritative-aware).
# ---------------------------------------------------------------------------
function Resolve-DnsRecordSafe
{
    ...
    $F_Doh = Resolve-DnsRecordDoH -Name $Name -Type $Type
    return [pscustomobject]@{
        Records    = @($F_Doh.Records)
        AllRecords = @($F_Doh.Records)
        Server     = $F_Doh.Server
        Error      = $F_Doh.Error
    }
}

...
<#
.SYNOPSIS
    Generates a CSV and HTML report of MFA coverage across Member users.
    By default only Enabled member accounts are evaluated; pass
    -IncludeDisabled to also fetch and enumerate disabled accounts. Guests
    are always excluded.

.DESCRIPTION
    Connects to Microsoft Graph and retrieves Member users (userType eq
    'Member', Guests excluded). The default Graph filter also requires
    accountEnabled eq true so disabled accounts are never fetched and never
    enumerated for authentication methods — this is the largest single cost
    in the run on tenants with many stale accounts. Pass -IncludeDisabled to
    restore the legacy behaviour and populate the Disabled card.
    For each user the registered authentication methods are inspected to
    classify MFA status (Modern Auth, Legacy Auth, or No MFA). Account
    activity, licensing and on-premises sync state are also captured.
...
.PARAMETER IncludeDisabled
    When specified, disabled member accounts are also fetched from Graph
    and enumerated for authentication methods. Default is enabled-only,
    which is significantly faster on tenants with many stale accounts. With
    -IncludeDisabled the Disabled card on the HTML report is populated;
    without it the Disabled card renders as N/A.

.PARAMETER Test
    When specified, only processes the first 10 Member users for quick testing.

.EXAMPLE
    .\ReportMemberMFA_v4.ps1 -InactiveDays 90

.EXAMPLE
    .\ReportMemberMFA_v4.ps1 -InactiveDays 30 -Test

.EXAMPLE
    .\ReportMemberMFA_v4.ps1 -InactiveDays 90 -IncludeDisabled
#>
...
if (-not $S_OutputPath)
{
    ...
    $S_OutputPath = Join-Path (Get-Location).Path "ReportMemberMFA_$S_Timestamp.csv"
}
...
try
{
    ...
    # v4: enabled-only by default. Disabled accounts dominate runtime cost on
    # large tenants because each one triggers a /authentication/methods call.
    # Pass -IncludeDisabled to restore the v3 behaviour and populate the
    # Disabled card on the HTML report.
    if ($IncludeDisabled)
    {
        $S_UserFilter = "userType eq 'Member'"
        Write-Host "  Scope: Enabled + Disabled (-IncludeDisabled specified)." -ForegroundColor DarkGray
    }
    else
    {
        $S_UserFilter = "userType eq 'Member' and accountEnabled eq true"
        Write-Host "  Scope: Enabled accounts only (default). Pass -IncludeDisabled to also enumerate disabled accounts." -ForegroundColor DarkGray
    }

    if ($Test)
    {
        Write-Host "[TEST MODE] Selecting 10 random Member users." -ForegroundColor Yellow
        $S_AllMembers = Get-MgUser -Filter $S_UserFilter -Property $S_UserProperties -All
        $S_Users = @($S_AllMembers | Get-Random -Count ([Math]::Min(10, ($S_AllMembers | Measure-Object).Count)))
    }
    else
    {
        $S_Users = Get-MgUser -Filter $S_UserFilter -Property $S_UserProperties -All
    }
    ...
    # v4: when -IncludeDisabled is NOT specified, disabled accounts were never
    # fetched from Graph. The Disabled card on the HTML report renders as
    # "N/A" with a tooltip explaining that disabled accounts were not
    # enumerated in this run. The other cards continue to count what was
    # actually returned (which, by definition, is the enabled population).
    if ($IncludeDisabled)
    {
        $S_DisabledCardValue  = [string]$S_DisabledCount
        $S_DisabledCardDetail = "$([math]::Round(($S_DisabledCount / [math]::Max($S_TotalMembers,1)) * 100, 1))% of total"
        $S_DisabledCardTitle  = ''
    }
    else
    {
        $S_DisabledCardValue  = 'N/A'
        $S_DisabledCardDetail = 'Not enumerated &mdash; run with <code>-IncludeDisabled</code> to populate'
        $S_DisabledCardTitle  = ' title="Disabled accounts were not fetched in this run. Pass -IncludeDisabled to populate this card."'
    }
...
<body>
    ...
    <div class="section">
        ...
        <div class="breakdown">
            ...
            <div class="card red"$S_DisabledCardTitle>
                <div class="label">Disabled</div>
                <div class="value">$S_DisabledCardValue</div>
                <div class="detail">$S_DisabledCardDetail</div>
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="user-filters">
            ...
            <select id="userFltActive" onchange="userApplyFilters()">
                <option value="Enabled" selected>Enabled (any)</option>
                <option value="">All (incl. Disabled)</option>
        ...
        function userApplyFilters() {
            ...
            rows.forEach(function(r){
                ...
                var ok = (!fau || r.dataset.auth === fau)
                      ...
                      && (!fa || (fa === 'Enabled' ? r.dataset.active !== 'Disabled' : r.dataset.active === fa))
        ...
        function userResetFilters() {
            ...
            document.getElementById('userFltActive').value = 'Enabled';
...
$S_AuthStrengthRows
    ...
    <div class="footer">
        CSV data: $(Split-Path $S_OutputPath -Leaf) | Report generated by ReportMemberMFA.ps1
    </div>
</body>
</html>
"@

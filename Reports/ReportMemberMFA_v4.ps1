...
try {
    ...
    if ($Test) {
        ...
        $S_AllMembers = Get-MgUser -Filter $S_UserFilter -Property $userProperties -All
        $users = @($S_AllMembers | Get-Random -Count ([Math]::Min(10, ($S_AllMembers | Measure-Object).Count)))
    }
    else {
        $users = Get-MgUser -Filter $S_UserFilter -Property $userProperties -All
    }
    ...
    if ($IncludeDisabled) {
        $S_DisabledCardValue  = [string]$disabledCount
        $S_DisabledCardDetail = "$([math]::Round(($disabledCount / [math]::Max($totalMembers,1)) * 100, 1))% of total"
        $S_DisabledCardTitle  = ''
    }
    else {

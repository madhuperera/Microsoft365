...
"@
	...
	$htmlReportFile = Join-Path $reportFolder ("ReviewInactiveGuestUsers_{0}.html" -f $S_Timestamp)
	...
	if ($disableReport.Count -gt 0) {
		$disableReportFile = Join-Path $reportFolder ("DisabledGuestUsers_{0}.csv" -f $S_Timestamp)

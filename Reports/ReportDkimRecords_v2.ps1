...
.DESCRIPTION
    ...
    for all non-onmicrosoft.com domains.
...
foreach ($S_DkimEntry in $S_DkimConfiguration)
{
    ...
    if ($S_DkimEnabled)
    {
        ...
        try
        {
            ...
            $S_Result1 = (Resolve-DnsRecordDoH -Type CNAME -Name $S_Selector1DNSName).Records | Select-Object -First 1
            if ($S_Result1)
            {
                $S_Selector1DnsHostName = $S_Result1.NameHost
                if ((Resolve-DnsRecordDoH -Type CNAME -Name $S_Selector1DnsHostName).Records)
                {
            ...
            {
                Write-Output "Error validating DNS for $S_Selector1DNSName. Please check your DNS Provider"
            }

            # Selector 2
            $S_Selector2DNSName = "selector2._domainkey.$S_DomainName"
            $S_Result2 = (Resolve-DnsRecordDoH -Type CNAME -Name $S_Selector2DNSName).Records | Select-Object -First 1
            if ($S_Result2)
            {
                $S_Selector2DnsHostName = $S_Result2.NameHost
                if ((Resolve-DnsRecordDoH -Type CNAME -Name $S_Selector2DnsHostName).Records)
                {
            ...
            {
                Write-Output "Error validating DNS for $S_Selector2DNSName. Please check your DNS Provider"
            }
        }
        catch
        {

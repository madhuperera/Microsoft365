...
foreach ($S_Domain in $S_AllDomains)
{
    ...
    $S_DmarcLookup = Resolve-DnsRecordDoH -Type TXT -Name $("_dmarc." + $S_Domain)
    $S_DmarcRecords = $S_DmarcLookup.Records | Where-Object { ($_.Strings -join '') -match '(?i)^\s*v=DMARC1' }

    if ($S_DmarcRecords)
    {
        $S_DmarcRecords | ForEach-Object { Write-Output ($_.Strings -join '') }
    }
    elseif ($S_DmarcLookup.Error)
    {
        Write-Output $S_DmarcLookup.Error
    }
    else
    {
        Write-Output "No DMARC (v=DMARC1) TXT record found for $S_Domain"
    }
}

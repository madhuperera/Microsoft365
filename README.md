...
| [`ReviewInactiveGuestUsers.ps1`](Reports/ReviewInactiveGuestUsers.ps1) | Reports on guest users who have not signed in within a configurable number of days. Supports a `Disable` mode to disable accounts after reporting. Exports to CSV and HTML. | `Microsoft.Graph.Users`; `User.Read.All`, `AuditLog.Read.All` (ReportOnly); additionally `User.ReadWrite.All` (Disable mode) |
| [`ReviewInactiveMemberUsers.ps1`](Reports/ReviewInactiveMemberUsers.ps1) | Reports on member users who have not signed in within a configurable number of days. Supports a `Disable` mode to disable accounts after reporting. Exports to CSV and HTML. | `Microsoft.Graph.Users`; `User.Read.All`, `AuditLog.Read.All` (ReportOnly); additionally `User.ReadWrite.All` (Disable mode) |
...
| [`ReportMemberMFA_v3.ps1`](Reports/ReportMemberMFA_v3.ps1) | Frozen reference copy of the v3 MFA coverage report, retained alongside the active version. Reports MFA coverage across Member users (Guests excluded), classifies each user as Modern Auth, Legacy Auth, or No MFA, and applies the Conditional Access policy scoring framework with inactivity-day filtering. Exports to CSV and HTML. | `Microsoft.Graph.Authentication`, `Microsoft.Graph.Users`, `Microsoft.Graph.Reports`, `Microsoft.Graph.Identity.SignIns`; `User.Read.All`, `AuditLog.Read.All`, `UserAuthenticationMethod.Read.All`, `Policy.Read.All` |
| [`ReportMemberMFA_v4.ps1`](Reports/ReportMemberMFA_v4.ps1) | **Current active version.** Reports MFA coverage across Member users. Defaults to enabled accounts only for performance; pass `-IncludeDisabled` to include disabled member accounts. Supports `-Test`. Exports to CSV and HTML. | `Microsoft.Graph.Authentication`, `Microsoft.Graph.Users`, `Microsoft.Graph.Reports`, `Microsoft.Graph.Identity.SignIns`; `User.Read.All`, `AuditLog.Read.All`, `UserAuthenticationMethod.Read.All`, `Policy.Read.All` |
...
**ReportMemberMFA companion documentation**

| File | Description |
|------|-------------|
| [`ReportMemberMFA-Guide.html`](Reports/ReportMemberMFA-Guide.html) | Operator guide for interpreting and actioning `ReportMemberMFA` report output, including the CA policy scoring framework, posture matrix, and FAQ. |
| [`ReportMemberMFA-CHANGELOG.md`](Reports/ReportMemberMFA-CHANGELOG.md) | Version history and change log for the `ReportMemberMFA` script series. |
| [`ReportMemberMFA-KNOWN-ISSUES.md`](Reports/ReportMemberMFA-KNOWN-ISSUES.md) | Known issues and planned work items for the `ReportMemberMFA` script series. |
